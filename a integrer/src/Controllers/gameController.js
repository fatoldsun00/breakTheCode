...
const joinGame = ( idGame,name ) => {
  try {
    ...
    if (!Object.values(game.joueurs).filter((joueur) => joueur.leader).length) game.joueurs[name]={leader: true}
/*
...
const reponseToCard = async (idGame,idCartes,response,name) => {
  try {
    ...
    if (name == game.tour.joueurActif) throw new AppError(401,'ERR_GAME_RESPONSE_IT_S_YOUR_TURN')
    //Test de la reponse
    ...
    //On ajoute le joueur au tableau des joueurs ayant jou̩ ce tour ci
...
const cleanGameForSend = (game,name) => {
...
function initGame(game) {
  ...
  for (joueur in game.joueurs){
    if (game.joueurs[joueur].leader) game.tour.joueurActif = joueur
  }
  if (!game.tour.joueurActif) game.tour.joueurActif = game.joueurs[0]
...
async function nextTurn  (idGame) {
  try {
    //Si toute le monde a repondu
    let joueursRestant = Object.keys(game.joueurs).filter(name => game.tour.aJouer.indexOf(name) < 0 )
    if (!joueursRestant.length) throw new AppError(422,'ERR_GAME_NEXT_TURN_WAIT_FOR_PLAYER')
    
    let game = getGame(idGame)
    //Gestion des cartes
      //on passe la carte selectionn̩ dans la defause
    ...
    if (game.pioche.length) {
      ...
      let joueurActifIndex = tabJoueurs.findIndex(joueur=>joueur==game.tour.joueurActif)
      joueurActifIndex++
